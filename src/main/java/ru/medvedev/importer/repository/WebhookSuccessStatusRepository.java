...
public interface WebhookSuccessStatusRepository extends JpaRepository<WebhookSuccessStatusEntity, Long>,
    ...
    @Query("select w from WebhookSuccessStatusEntity w " +
            "where w.webhookId = :webhookId " +
            "and w.bank = :bank")
    List<WebhookSuccessStatusEntity> findAllByBankAndWebhookId(Bank bank, Long webhookId);
    ...
    boolean existsByWebhookIdAndBankAndType(Long webhookId, Bank bank, WebhookType type);
}
